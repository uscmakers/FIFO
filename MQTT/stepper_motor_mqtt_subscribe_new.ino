...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "door/closed" && msg == "1") {
    ...
    // ---- Minimal change: run fixed steps (blocking) to move the belt ≈30 cm ----
    // Adjust this number to tune exact travel distance/time for your pulley.
    const long FIXED_STEPS = 9776; // example value (tune as needed)
    myStepper.step(FIXED_STEPS);
    Serial.println("Stepper motor STOPPED. Completed motion.");
    // ------------------------------------------------------------------------
  }
}
